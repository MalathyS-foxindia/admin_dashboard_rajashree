...
class SupabaseService {
  ...
  Future<List<Map<String, dynamic>>?> getDailySalesStats(
    DateTime date,
    String? dsourceFilter,
  ) async {
    print(
      'Fetching daily sales stats for date: $date with dsourceFilter: $dsourceFilter',
    );
    try {
  ...
  Future<List<Map<String, dynamic>>?> getWeeklySalesStats() async {
    try {
      final response = await supabase.rpc('get_weekly_sales_stats');
      if (response is List) {
        return List<Map<String, dynamic>>.from(response);
      }
      return [];
  ...
  Future<int> getTotalCustomers() async {
    try {
      final response = await supabase.from('customers').select('customer_id');
